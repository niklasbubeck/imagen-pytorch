    ...
    def __init__(
        ...
        *,
        ...
        self.flatten_block = nn.Sequential(
            ...
            nn.Conv2d(mid_dim, text_embed_dim, 1, 1, 0),
            nn.Flatten(),
        )
    ...
    def forward(self, xt, t, images=None, **kwargs):
        ...
        Returns:
            out (torch.tensor): A tensor of output.
                ...
                dtype = torch.float32
        """
        del kwargs["text_embeds"]
        style_emb = self.encoder(images)
        text_masks = lambda: torch.any(style_emb != 0., dim = -1)
        kwargs['text_mask'] = text_masks
        out = self.unet(xt, t, text_embeds=style_emb, **kwargs)
        return out
...
class Imagen(nn.Module):
    def __init__(
        ...
        *,
        ...
        for ind, one_unet in enumerate(unets):
            print(type(one_unet))
            assert isinstance(one_unet, (DiffusionAutoEncoders3D, Unet, Unet3D, NullUnet, DiffusionAutoEncoders))
            is_first = ind == 0

            if isinstance(one_unet,(DiffusionAutoEncoders3D, DiffusionAutoEncoders)):
    ...
    def sample(
        ...
        for unet_number, unet, channel, image_size, frame_dims, noise_scheduler, pred_objective, dynamic_threshold, unet_cond_scale, unet_init_images, unet_skip_steps in tqdm(zip(range(1, num_unets + 1), self.unets, self.sample_channels, self.image_sizes, all_frame_dims, self.noise_schedulers, self.pred_objectives, self.dynamic_thresholding, cond_scale, init_images, skip_steps), disable = not use_tqdm):
            ...
            with context:
                ...
                if isinstance(unet, (DiffusionAutoEncoders3D, DiffusionAutoEncoders)):
                    text_embeds = unet.encoder(encoder_images)
                    text_masks = lambda: torch.any(text_embeds != 0., dim = -1)
    ...
    def p_losses(
        self,
        unet: Union[ DiffusionAutoEncoders3D, DiffusionAutoEncoders, Unet, Unet3D, NullUnet, DistributedDataParallel],
        x_start,
        times,
        *,
        noise_scheduler,
    ...
    def forward(
        ...
        unet: Union[DiffusionAutoEncoders3D, DiffusionAutoEncoders, Unet, Unet3D, NullUnet, DistributedDataParallel] = None,
        texts: List[str] = None,
        text_embeds = None,
        text_masks = None,
        unet_number = None,
